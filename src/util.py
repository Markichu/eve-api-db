...
HTTPX_LIMITS = httpx.Limits(
    max_keepalive_connections=CONCURRENT_REQUESTS, max_connections=CONCURRENT_REQUESTS
)
...
async def connect_to_db() -> asyncpg.connection.Connection:
    conn = await asyncpg.connect(
        host="localhost", database="eve", user="postgres", password="password"
    )
    return conn
...
async def esi_call(
    ...
    yield (
        response := await GLOBAL_SESSION.get(
            endpoint_url, params=params, headers=headers, timeout=10
        )
    )
    ...
    other_requests = (
        GLOBAL_SESSION.get(endpoint_url, params=params | {"page": i}, headers=headers)
    ...
    try:
        async for response in esi_call(method_url, params=params, headers=headers):
            ...
            yield {
                ...
                "x-esi-error-limit-remain": response.headers.get(
                    "x-esi-error-limit-remain", 0
                ),
                "x-esi-error-limit-reset": response.headers.get(
            ...
            if response.status_code != 200:
                raise Exception(
                    f"ESI call failed with status code {response.status_code}"
                )
            if "application/json" not in response.headers.get("Content-Type", ""):
                raise Exception(
                    f"ESI call failed with non-JSON response: {response.text}"
                )
